...
public class DTOInstArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        if (IDTO.class.isAssignableFrom(parameter.getParameterType()) && parameter.getParameterType().isInterface()) {
            return true;
        }
        //List中有DTO泛型参数
        if (List.class.isAssignableFrom(parameter.getParameterType())) {
            if (!(parameter.getGenericParameterType() instanceof ParameterizedTypeImpl)) {
                return false;
            }
            Type actualTypeArgument = ((ParameterizedTypeImpl) parameter.getGenericParameterType()).getActualTypeArguments()[0];
            if (!(actualTypeArgument instanceof Class)) {
                return false;
            }
            Class actualTypeClass = (Class) actualTypeArgument;
            if (IDTO.class.isAssignableFrom(actualTypeClass) && actualTypeClass.isInterface()) {
                return true;
            }
        }
        return false;
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        if (clazz != null && IDTO.class.isAssignableFrom(clazz)) {
            ...
            Class actualTypeArgument = (Class) ((ParameterizedTypeImpl) parameter.getGenericParameterType()).getActualTypeArguments()[0];
            return getDTOList(actualTypeArgument, webRequest, parameter);
        }
        return UNRESOLVED;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    protected <T extends IDTO> List<T> getDTOList(Class<T> clazz, NativeWebRequest webRequest, MethodParameter parameter) {
        //构建fields Map，用于getParamValuesAndConvert()
        Map<String, Class<?>> fields = getFieldsMap(clazz);
        // 实例化一个DTO数据对象
        List<T> dtoList4Restful = getDTOList4Restful(fields, webRequest, parameter, clazz);
        asetErrorMsg(dtoList4Restful, parameter);
        return dtoList4Restful;
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private <T extends IDTO>  List<T> getDTOList4Restful(Map<String, Class<?>> fields, NativeWebRequest webRequest, MethodParameter parameter, Class<T> clazz) {
        // 实例化一个DTO数据对象
        ...
        try {
            ...
            ((JSONArray)valid).forEach(obj -> {
                ...
                dtoList.add(DTOUtils.proxyInstance(dto, clazz));
            });
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private <T extends IDTO> void asetErrorMsg(List<T> list, MethodParameter parameter){
        Validated validated = parameter.getParameter().getAnnotation(Validated.class);
        //有Validated注解则进行校验
        if(validated != null) {
            list.stream().forEach(t-> t.aset(IDTO.ERROR_MSG_KEY, BeanValidator.validator(t, validated.value())));
        }
    }
...
//		}
        ...
        DTO dto = buildDtoByParameterMap(webRequest.getParameterMap(), clazz, fields);
        //body覆盖queryParam
    ...
    /**
     * 根据requestParameterMap构建DTO
     *
     * @param paramsMap
     * @return
     */
    private <T extends IDTO> DTO buildDtoByParameterMap(Map<String, String[]> paramsMap, Class<T> clazz, Map<String, Class<?>> fields) {
        DTO dto = new DTO();
        // 填充值
     ...
     * @return
     */
    private Map<String, Class<?>> getFieldsMap(Class<?> clazz) {
